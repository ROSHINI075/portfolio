...
(function injectSpinnerCSS() {
  ...
  s.textContent = `
    ...
    .skill-cat-card.hidden { display: none !important; }
  `;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ICON_MENU  = `<span></span><span></span><span></span>`;
  const ICON_CLOSE = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="margin-left:-4px"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`;
  ...
  if (typingEl) {
    const words = ['Software Developer', 'MCA Graduate', 'Full-Stack Developer', 'AI Enthusiast'];
  ...
  /* ── 6. EXPERIENCE TABS ────────────────────────── */
  // The layout doesn't use tabs anymore, but we keep this empty if needed.
  ...
  const revealObs = new IntersectionObserver((entries, obs) => {
    entries.forEach(entry => {
      ...
      entry.target.classList.add('active'); // CSS uses .active to fade in!
